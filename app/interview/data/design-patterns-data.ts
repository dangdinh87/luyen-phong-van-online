...
export const DESIGN_PATTERNS_DATA: QAItem[] = [
  ...
  },
  {
    ...
    a: "Builder tách quá trình xây dựng object phức tạp thành các bước riêng biệt, cho phép tạo nhiều biểu diễn khác nhau của cùng một object. Ví dụ TypeScript với method chaining: `new QueryBuilder().select('*').from('users').where('age > 18').orderBy('name').limit(10).build()` — thay vì `new Query('*', 'users', 'age > 18', 'name', 10, undefined, undefined)`. Ưu điểm so với constructor nhiều tham số: không cần nhớ thứ tự tham số, tham số optional không cần truyền `undefined`, code readable hơn, có thể validate từng bước. Dùng khi: object có nhiều optional parameter (Telescoping Constructor anti-pattern); khi quá trình khởi tạo phức tạp có nhiều bước; khi cần tạo nhiều variant của cùng object. Trong Go, Builder thường implement bằng functional options pattern: `NewServer(WithPort(8080), WithTimeout(30*time.Second))`.",
    q_en: "What is the Builder pattern? What advantages does it have over constructors with many parameters?",
    a_en: "Builder separates the construction of a complex object into discrete steps, enabling different representations of the same object. TypeScript example with method chaining: `new QueryBuilder().select('*').from('users').where('age > 18').orderBy('name').limit(10).build()` — instead of `new Query('*', 'users', 'age > 18', 'name', 10, undefined, undefined)`. Advantages over many-parameter constructors: no need to remember parameter order, optional parameters don't require passing `undefined`, code is more readable, and each step can be validated individually. Use it when an object has many optional parameters (Telescoping Constructor anti-pattern), when initialization involves multiple complex steps, or when you need to produce different variants of the same object. In Go, Builder is commonly implemented via the functional options pattern: `NewServer(WithPort(8080), WithTimeout(30*time.Second))`.",
  },
  {
    id: 2113,
    category: "Design Patterns",
  ...
  },
  {
    ...
    a: "DI là technique mà object nhận dependencies từ bên ngoài thay vì tự tạo — là cơ chế triển khai DIP. Ba loại DI: (1) Constructor Injection (phổ biến nhất): `constructor(private userRepo: UserRepository) {}` — dependency bắt buộc, immutable sau khi inject; (2) Setter/Property Injection: `setRepository(repo: UserRepository)` — cho optional dependency; (3) Method Injection: dependency truyền vào method call — dùng khi dependency chỉ cần cho một operation. Framework DI: NestJS dùng IoC container với decorators `@Injectable()`, `@Inject()` — tự động resolve dependency tree. Trong Go không có framework DI phổ biến, thường dùng manual DI qua `wire` (Google) hoặc constructor functions. Lợi ích: loose coupling, dễ test (inject mock), dễ swap implementation. Anti-pattern: Service Locator (class tự gọi `container.get('UserRepo')`) — tạo hidden dependency.",
    q_en: "What is Dependency Injection (DI)? What are the types of DI and what frameworks support it?",
    a_en: "DI is the technique where an object receives its dependencies from the outside rather than creating them — it is the mechanism that implements DIP. Three types of DI: (1) Constructor Injection (most common): `constructor(private userRepo: UserRepository) {}` — mandatory dependency, immutable after injection; (2) Setter/Property Injection: `setRepository(repo: UserRepository)` — for optional dependencies; (3) Method Injection: dependency is passed into a method call — used when a dependency is only needed for one operation. DI frameworks: NestJS uses an IoC container with `@Injectable()` and `@Inject()` decorators that automatically resolve the dependency tree. Go has no dominant DI framework; manual DI via `wire` (Google) or plain constructor functions is the norm. Benefits: loose coupling, easy testing (inject mocks), easy implementation swapping. Anti-pattern: Service Locator (a class calling `container.get('UserRepo')`) — creates hidden dependencies.",
  },
  {
    id: 2116,
    category: "Design Patterns",
  ...
  // ── Structural ─────────────────────────────────────────────────────────────
  {
    ...
    a: "Adapter cho phép các interface incompatible làm việc cùng nhau bằng cách bọc một object trong wrapper cung cấp interface mà client kỳ vọng. Ví dụ thực tế: tích hợp third-party payment SDK — `PayPalSDK` có method `makePayment(amount, currency)` nhưng app của bạn kỳ vọng `PaymentProvider` interface có `charge(cents: number): Promise<Receipt>`. Ta tạo `PayPalAdapter implements PaymentProvider { charge(cents) { return this.sdk.makePayment(cents/100, 'USD') } }`. Hai loại Adapter: Object Adapter (composition, prefer này) và Class Adapter (multiple inheritance — không có trong TS/JS). Dùng khi: tích hợp legacy code hoặc third-party library có interface không phù hợp; khi không thể sửa source code của class cần adapt. Trong frontend, Adapter thường dùng để normalize API response format khác nhau về một schema thống nhất.",
    q_en: "What is the Adapter pattern? Real-world example in TypeScript?",
    a_en: "Adapter allows incompatible interfaces to work together by wrapping an object in a wrapper that exposes the interface the client expects. Real-world example: integrating a third-party payment SDK — `PayPalSDK` has a `makePayment(amount, currency)` method, but your app expects a `PaymentProvider` interface with `charge(cents: number): Promise<Receipt>`. You create `PayPalAdapter implements PaymentProvider { charge(cents) { return this.sdk.makePayment(cents/100, 'USD') } }`. Two variants: Object Adapter (composition — preferred) and Class Adapter (multiple inheritance — not available in TS/JS). Use it when integrating legacy code or a third-party library whose interface doesn't match yours, or when you can't modify the source of the class being adapted. In frontend development, Adapter is often used to normalize different API response formats into a unified schema.",
  },
  {
    id: 2118,
    category: "Design Patterns",
    ...
    a: "Bridge tách abstraction khỏi implementation để cả hai có thể thay đổi độc lập — giải quyết 'Cartesian product' explosion khi có nhiều dimension. Ví dụ: `Shape` (Circle, Square) × `Renderer` (SVGRenderer, CanvasRenderer) = 4 class nếu dùng inheritance, nhưng với Bridge chỉ cần 2+2 class: `class Circle { constructor(private renderer: Renderer) {} draw() { this.renderer.renderCircle(this.radius) } }`. Khác Adapter: Adapter giải quyết incompatibility giữa existing interfaces (fix sau); Bridge thiết kế từ đầu để tách abstraction-implementation (design upfront). Dùng Bridge khi: muốn tránh class explosion do kết hợp nhiều dimension; khi abstraction và implementation cần thay đổi độc lập; khi muốn share implementation giữa nhiều object. Không dùng khi: chỉ có một dimension biến đổi — over-engineering.",
    q_en: "What is the Bridge pattern? How does it differ from Adapter?",
    a_en: "Bridge decouples abstraction from implementation so that the two can vary independently — solving the class explosion that occurs when combining multiple dimensions. Example: `Shape` (Circle, Square) × `Renderer` (SVGRenderer, CanvasRenderer) = 4 classes with inheritance, but Bridge only requires 2 + 2 classes: `class Circle { constructor(private renderer: Renderer) {} draw() { this.renderer.renderCircle(this.radius) } }`. Difference from Adapter: Adapter resolves incompatibility between existing interfaces (a retrofit fix); Bridge is designed upfront to separate abstraction from implementation. Use Bridge when you want to avoid class explosion from combining multiple dimensions, when abstraction and implementation need to evolve independently, or when you want to share an implementation across multiple objects. Avoid it when only one dimension varies — that's over-engineering.",
  },
  {
    id: 2119,
    category: "Design Patterns",
    ...
    a: "Composite cho phép compose objects thành tree structures để biểu diễn part-whole hierarchies — client xử lý individual objects và compositions đồng nhất qua cùng một interface. Ví dụ: file system — `interface FileSystemItem { getSize(): number }` với `class File implements FileSystemItem` và `class Folder implements FileSystemItem { private children: FileSystemItem[]; getSize() { return this.children.reduce((sum, c) => sum + c.getSize(), 0) } }`. Client gọi `getSize()` trên cả File và Folder mà không cần biết loại. Trong React, component tree là ứng dụng tự nhiên của Composite — component con và cha đều là React component. Dùng khi: cần biểu diễn tree hierarchy; khi muốn client treat leaf và composite đồng nhất. Không dùng khi: hierarchy phẳng hoặc interface chung quá generic, khó type-safe.",
    q_en: "What is the Composite pattern? When should you use it?",
    a_en: "Composite lets you compose objects into tree structures to represent part-whole hierarchies — clients treat individual objects and compositions uniformly through a single interface. Example: a file system — `interface FileSystemItem { getSize(): number }` with `class File implements FileSystemItem` and `class Folder implements FileSystemItem { private children: FileSystemItem[]; getSize() { return this.children.reduce((sum, c) => sum + c.getSize(), 0) } }`. The client calls `getSize()` on both files and folders without needing to know the type. In React, the component tree is a natural application of Composite — leaf and parent components are both React components. Use it when you need to represent a tree hierarchy or when you want clients to treat leaves and composites uniformly. Avoid it when the hierarchy is flat or when a shared interface is too generic to be type-safe.",
  },
  {
    id: 2120,
    category: "Design Patterns",
  ...
  },
  {
    ...
    a: "Strategy định nghĩa family of algorithms, encapsulate mỗi cái, và làm chúng interchangeable — cho phép algorithm thay đổi độc lập với client sử dụng nó. Ví dụ TypeScript: `interface SortStrategy { sort(data: number[]): number[] }` với `BubbleSort`, `QuickSort`, `MergeSort` implements — `Sorter` nhận strategy qua constructor và gọi `this.strategy.sort(data)`. So với if/else: Strategy tuân thủ OCP — thêm algorithm mới không cần sửa `Sorter`; if/else vi phạm OCP, dài dần theo thời gian. Dùng Strategy khi: có nhiều variant của một algorithm; khi muốn swap algorithm tại runtime; khi muốn isolate algorithm logic để dễ test riêng từng cái. Trong React, strategy hay xuất hiện dưới dạng render props hoặc component props nhận function: `<DataTable sortFn={quickSort} filterFn={fuzzyFilter} />`. Không dùng khi chỉ có 2-3 strategy ít thay đổi — if/else đủ đơn giản hơn.",
    q_en: "What is the Strategy pattern? How does it compare to if/else and switch/case?",
    a_en: "Strategy defines a family of algorithms, encapsulates each one, and makes them interchangeable — allowing the algorithm to vary independently from the client that uses it. TypeScript example: `interface SortStrategy { sort(data: number[]): number[] }` with `BubbleSort`, `QuickSort`, and `MergeSort` implementations — `Sorter` receives the strategy via constructor and calls `this.strategy.sort(data)`. Compared to if/else: Strategy follows OCP — adding a new algorithm doesn't require modifying `Sorter`; if/else violates OCP and grows longer over time. Use Strategy when there are multiple variants of an algorithm, when you want to swap the algorithm at runtime, or when you want to isolate algorithm logic for independent testing. In React, Strategy often appears as render props or function props: `<DataTable sortFn={quickSort} filterFn={fuzzyFilter} />`. Skip it when there are only 2–3 strategies that rarely change — a simple if/else is more readable.",
  },
  {
    id: 2127,
    category: "Design Patterns",
    ...
    a: "Command encapsulate một request thành object, cho phép parameterize clients với different requests, queue/log requests, và support undoable operations. Cấu trúc: `interface Command { execute(): void; undo(): void }` — `ConcreteCommand` lưu receiver và params cần thiết để thực hiện và hoàn tác. Triển khai Undo/Redo: duy trì `historyStack` và `redoStack` — `execute()` push command vào history; `undo()` pop từ history, gọi `command.undo()`, push vào redo stack. Ứng dụng thực tế: text editor (Ctrl+Z), transaction trong database, task queue (Bull, BullMQ), HTTP request retry. Trong Redux, mỗi `dispatch(action)` là Command — `redux-undo` library implement Undo/Redo bằng Command pattern. Dùng khi: cần undo/redo, transaction, audit log, retry; khi muốn queue và schedule requests. Không dùng khi: simple method call đủ dùng — Command overhead không cần thiết.",
    q_en: "What is the Command pattern? How is Undo/Redo implemented with it?",
    a_en: "Command encapsulates a request as an object, allowing clients to be parameterized with different requests, to queue or log requests, and to support undoable operations. Structure: `interface Command { execute(): void; undo(): void }` — a `ConcreteCommand` stores the receiver and the parameters needed to execute and reverse the action. Undo/Redo implementation: maintain a `historyStack` and a `redoStack` — `execute()` pushes the command onto history; `undo()` pops from history, calls `command.undo()`, and pushes onto the redo stack. Real-world uses: text editors (Ctrl+Z), database transactions, task queues (Bull, BullMQ), HTTP request retry. In Redux, every `dispatch(action)` is a Command — the `redux-undo` library implements Undo/Redo using this pattern. Use it when you need undo/redo, transactions, audit logs, or retries, or when you want to queue and schedule requests. Skip it when a simple method call suffices — Command overhead isn't always justified.",
  },
  {
    id: 2128,
    category: "Design Patterns",
  ...
  },
  {
    ...
    a: "Chain of Responsibility cho phép pass request qua chain of handlers — mỗi handler quyết định xử lý hoặc pass cho handler tiếp theo. Express.js middleware là ví dụ hoàn hảo: `app.use(authMiddleware, rateLimitMiddleware, validationMiddleware, routeHandler)` — mỗi middleware gọi `next()` để pass request. Ví dụ TypeScript: `interface Handler { setNext(h: Handler): Handler; handle(request: number): string | null }` — `abstract class AbstractHandler { protected nextHandler?: Handler; setNext(h) { this.nextHandler = h; return h } handle(r) { return this.nextHandler?.handle(r) ?? null } }`. Dùng khi: nhiều object có thể handle request và muốn decouple sender từ receiver; khi handlers nên được configurable hoặc reorderable tại runtime. Khác Observer: Chain of Responsibility chỉ một handler xử lý; Observer notify tất cả. Không dùng khi: chain quá dài gây performance issue; khi không rõ handler nào sẽ xử lý gây khó debug.",
    q_en: "What is Chain of Responsibility? How is it applied in Express.js middleware?",
    a_en: "Chain of Responsibility allows a request to pass through a chain of handlers — each handler decides whether to process it or pass it to the next. Express.js middleware is a perfect example: `app.use(authMiddleware, rateLimitMiddleware, validationMiddleware, routeHandler)` — each middleware calls `next()` to pass the request along. TypeScript example: `interface Handler { setNext(h: Handler): Handler; handle(request: number): string | null }` — `abstract class AbstractHandler { protected nextHandler?: Handler; setNext(h) { this.nextHandler = h; return h } handle(r) { return this.nextHandler?.handle(r) ?? null } }`. Use it when multiple objects may handle a request and you want to decouple the sender from the receiver, or when handlers should be configurable and reorderable at runtime. Difference from Observer: Chain of Responsibility has only one handler process the request; Observer notifies all observers. Avoid it when the chain is too long and causes performance issues, or when it's unclear which handler will act, making debugging difficult.",
  },
  {
    id: 2130,
    category: "Design Patterns",
  ...
  },
  {
    ...
    a: "Iterator cung cấp cách access tuần tự các elements của collection mà không expose underlying representation. JavaScript triển khai Iterator Protocol tự nhiên: object là iterator nếu có `next()` method trả về `{ value, done }`. Iterable protocol: object có `[Symbol.iterator]()` method trả về iterator. Ví dụ custom iterator: `class Range { constructor(start, end) {...} [Symbol.iterator]() { let current = this.start; return { next: () => current <= this.end ? { value: current++, done: false } : { value: undefined, done: true } } } }` — có thể dùng trong `for...of`, spread `[...range]`, destructuring. Generator functions (`function*`) là sugar syntax tạo iterator/iterable. Dùng khi: cần traverse collection mà không expose structure; khi muốn lazy evaluation (generate values on demand — tiết kiệm memory cho large datasets). Trong React, Suspense và streaming server rendering dùng iterator concept.",
    q_en: "What is the Iterator pattern? How does the JavaScript Iterator Protocol work?",
    a_en: "Iterator provides a way to sequentially access elements of a collection without exposing its underlying representation. JavaScript implements the Iterator Protocol natively: an object is an iterator if it has a `next()` method that returns `{ value, done }`. The Iterable protocol: an object has a `[Symbol.iterator]()` method that returns an iterator. Custom iterator example: `class Range { constructor(start, end) {...} [Symbol.iterator]() { let current = this.start; return { next: () => current <= this.end ? { value: current++, done: false } : { value: undefined, done: true } } } }` — can be used in `for...of`, spread `[...range]`, and destructuring. Generator functions (`function*`) are syntactic sugar for creating iterators/iterables. Use it when you need to traverse a collection without exposing its structure, or when you want lazy evaluation (generating values on demand to save memory with large datasets). In React, Suspense and streaming server rendering use iterator concepts.",
  },
  {
    id: 2133,
    category: "Design Patterns",
