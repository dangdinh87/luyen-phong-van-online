...
export const FPT_RESEARCHED_DATA: QAItem[] = [
  ...
  },
  {
    ...
    q_en: "How do you handle race conditions in async useEffect? Example: user types in a search box then clears it, and the old request resolves after the new one.",
    a_en: "A race condition happens when multiple async requests are in-flight and responses arrive out of order, causing the UI to display stale data. The most common fix is using AbortController in the useEffect cleanup: create a controller, pass its signal to fetch, and abort in the cleanup. Example: `const controller = new AbortController(); fetch(url, { signal: controller.signal }); return () => controller.abort()`. Alternatively, use TanStack Query, which handles caching, request deduplication, and cancellation automatically."
  },
  {
    id: 938,
    category: "React",
  ...
  },
  {
    ...
    a_en: "React.memo() is a higher-order component that wraps a functional component to skip re-rendering when props haven't changed, using shallow comparison. Use it when: the child has an expensive render, and its parent re-renders frequently while its props rarely change. Note: shallow comparison only checks references — if you pass a new object or function on every render, memo is ineffective. Always combine with useCallback and useMemo. Do not memo every component; the comparison itself has a cost."
  },
  ...
  // TypeScript Gotchas
  {
    ...
    a_en: "Generics are parameterized types that let you write functions or classes that work with multiple types while preserving type safety. The syntax `<T extends Base>` means T must be assignable to Base — i.e., T must have at least the properties Base requires. Example: `<T extends { name: string }>` guarantees T always has a `name: string` property, but may have additional ones. Generics are invaluable for writing API wrappers, utility functions, and reusable React component props that need to work with different data shapes."
  },
  {
    id: 950,
    category: "TypeScript",
  ...
  },
  {
    ...
    a_en: "CSS Grid `template-areas` lets you name regions in the grid and assign elements to them via `grid-area`, creating a visual and readable layout definition. Example: `grid-template-areas: 'header header' 'sidebar main' 'footer footer'` with each element assigned `grid-area: header`, `grid-area: sidebar`, etc. For responsive behavior, simply use a `@media` query to change `grid-template-areas` — no HTML changes needed, just rearrange the named areas. This is the biggest advantage: completely change the layout with CSS alone while keeping the HTML unchanged — easy to maintain and understand."
  },
  {
    id: 958,
    category: "CSS",
  ...
  },
  {
    ...
    a_en: "An error boundary is a special component that catches JavaScript errors in its child component tree, preventing a full app crash (blank screen). Implementation: a class component with `componentDidCatch` and `static getDerivedStateFromError` — when an error occurs, it renders a fallback UI instead of crashing. Note: error boundaries do NOT catch errors in: event handlers, async code (setTimeout, Promises), server-side rendering, or the boundary itself. Common use case: wrap each major page section independently — if one section errors, the rest of the page still works. Include a retry button so users can recover."
  },
  {
    id: 983,
    category: "React",
